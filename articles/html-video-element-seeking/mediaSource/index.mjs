...
import {
    ...
    fetchBuffer,
    waitVideoIsLoaded,
    seekVideoSequentiallyToTimestamps,
} from "../utils.mjs";
...
document.querySelector("#addSource").addEventListener("click", () => {
    document
        ...
        .appendChild(document.createElement("video-source-selector"));
});
...
async function test() {
    ...
    const runInParallel = document.querySelector(
        "[name=runInParallel]"
    ).checked;
    const preloadToBlobStorage = document.querySelector(
        "[name=preloadToBlobStorage]"
    ).checked;
    const videoSourcesToCompare = document.querySelectorAll(
        "video-source-selector"
    );

    async function getSeekingPerformanceStatsForVideoSource(
        videoElement,
        timestamps,
        mediaUrl
    ) {
        const globalStartTime = new Date().getTime();
        const results = await seekVideoSequentiallyToTimestamps(
            videoElement,
            timestamps
        );

        const averageFrameSeekTime = average(results);
    ...
    const videoElements = await Promise.all(
        Array.from(videoSourcesToCompare).map(async (videoSourceElement) => {
            const mediaUrl = videoSourceElement.value;
            const videoElement = document.createElement("video");
            videoElement.src = preloadToBlobStorage
                ? await getBlobStorageUrl(mediaUrl)
                : mediaUrl;
    ...
    if (runInParallel) {
        results = await Promise.all(
            Array.from(videoElements).map(async ([videoElement, mediaUrl]) => {
                if (showPreview) {
                    previews.appendChild(videoElement);
                }
                return getSeekingPerformanceStatsForVideoSource(
                    videoElement,
                    timestamps,
                    mediaUrl
                );
        ...
        for (let i = 0; i < videoElements.length; i++) {
            ...
            const result = await getSeekingPerformanceStatsForVideoSource(
                videoElement,
                timestamps,
                mediaUrl
            );
    ...
    const totalFramesGenerated =
        timestamps.length * videoSourcesToCompare.length;
    const report = {
    ...
    resultTable.innerHTML = `
        ...
        <tbody>
            ${results
                ...
                <tr>
                    <td>
                       <a href="${result.mediaUrl}">${getFileNameFromUrl(
                        result.mediaUrl
                    )}</a>
                    </td>
    ...
    results.forEach((result) => {
        ...
        new Chart(ctx, {
            type: "line",
            data: {
                labels: result.results.map((result, index) => index),
                datasets: [
                    {
                        label: `${getFileNameFromUrl(result.mediaUrl)} Seek Time (ms)`,
                        data: result.results.map((v) => v),
                        borderColor: "blue",
                        fill: false,
                        tension: 0.1,
                    },
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        title: { display: true, text: "Seek Time (ms)" },
                    },
                },
            },
        });
