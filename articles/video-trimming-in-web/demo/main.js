...
async function main() {
    ...
    const regionsDomElement = document.getElementById("regions");
    regionsDomElement.value = JSON.stringify([
        [1000, 4000],
        [7000, 10000],
        [12000, 19000],
        [20000, 25000],
    ...
    const regions = JSON.parse(regionsDomElement.value);
    ...
    for (let streamI = 0; streamI < istreams.length; streamI++) {
        ...
        const decoder = new Decoder({
            output: (frame) => {
                // check frame.timestamp within the regions
                // and close the frame if not in the region
                ...
                for (let i = 0; i < regions.length; i++) {
                    ...
                    if (frameTimestamp >= region[0] && frameTimestamp <= region[1]) {
                        inRegion = true;
                        regionNumber = i;
                        break;
                    }
                ...
                let trimmedTimestamp = frameTimestamp - regions[regionNumber][0];

                // set timestamp to frame as a sum of durations of all previous regions plus frameTimestamp - startTime of found region
                let timestamp = 0;
                for (let i = 0; i < regionNumber; i++) {
                    timestamp += regions[i][1] - regions[i][0];
                }
                timestamp += trimmedTimestamp;
                const newFrameTimeStamp = timestamp * 1000;
                frame.timestamp = timestamp * 1000;
